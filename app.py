...
st.title("⚔️ 판타지 텍스트 RPG 게임 마스터 (모바일 스크롤 튐 해결 버전)")
st.markdown(
    "Google Gemini API와 Streamlit을 연동하여, 강제 새로고침을 없애고 모바일"
    " 스크롤 안정성을 높인 텍스트 RPG 공간입니다."
)
...
# 🖥️ [1. 메인 채팅 입력 및 응답 처리 (사이드바보다 먼저 처리하여 스탯 즉시 반영)]
...
# 메인 채팅 로직을 먼저 수행하여 스탯 변동을 즉시 세션에 반영
if api_key_input:
  try:
    if (
      ...
      with st.chat_message("assistant"):
        with st.spinner("게임 마스터가 다음 상황을 계산 중입니다..."):
          try:
            ...
            # JSON 데이터를 파싱하여 사이드바 상태값 즉시 갱신 (st.rerun 없음)
            match = re.search(
...
# ⚙️ [2. 좌측 사이드바: 최신 스탯이 반영된 상태창 렌더링]
