...
def clean_tags(text):
    ...
    text = re.sub(r"\[GOLD_CHANGE:\s*-?\d+\s*\]", "", text, flags=re.DOTALL)
    text = re.sub(r"\[STAT_CHANGE:\s*(\{.*?\})\s*\]", "", text, flags=re.DOTALL)
    return text.strip()
...
def process_combat_turn(action_type, skill_obj=None):
    ...
    if enemy["hp"] <= 0:
        ...
        if random.randint(1, 100) <= evasion:
            ...
            log_parts.append(
                f"💥 **{enemy['name']}**의 반격! {edmg}의 피해를 입혔습니다. (내 HP:"
                f" {player['hp']}/{player['max_hp']})"
            )
    ...
    # 2단계: 직업 선택
        ...
        if (
            ...
            sys_inst = (
                ...
                "5. 플레이어의 체력, 마나, 경험치에 변동이 생기는 경우 (함정 피격, 휴식, 퀘스트 완료 등) 반드시 응답에 [STAT_CHANGE: {\"hp\": 변동치, \"mp\": 변동치, \"exp\": 변동치}] 태그를 포함하세요. (변동이 없는 항목은 생략 가능하며, 예: HP 15 감소 시 [STAT_CHANGE: {\"hp\": -15}])\n"
                "6. 플레이어가 전투를 유발하는 행동을 하면 응답 끝에 반드시 [START_COMBAT: {\"name\": \"적 이름\", \"hp\": 45, \"atk\": 11}] 태그를 넣어 적을 생성하세요.\n"
                "7. 항상 응답 마지막에 3~4개의 행동 선택지를 [CHOICES: [\"선택지1\", \"선택지2\"]] 형태로 제시하세요."
            )
                ...
                with st.chat_message("assistant"):
                    with st.spinner("게임 마스터가 처리 중입니다..."):
                        try:
                            ...
                            # 3. 스탯(HP, MP, EXP) 변동 태그 처리 (새로 추가된 실시간 동기화 로직)
                            stat_change_match = re.search(
                                r"\[STAT_CHANGE:\s*(\{.*?\})\s*\]", bot_reply, re.DOTALL
                            )
                            if stat_change_match:
                                try:
                                    sdata = json.loads(stat_change_match.group(1))
                                    hp_delta = sdata.get("hp", 0)
                                    mp_delta = sdata.get("mp", 0)
                                    exp_delta = sdata.get("exp", 0)

                                    if hp_delta != 0:
                                        stats["hp"] = max(0, min(stats["max_hp"], stats["hp"] + int(hp_delta)))
                                    if mp_delta != 0:
                                        stats["mp"] = max(0, min(stats["max_mp"], stats["mp"] + int(mp_delta)))

                                    stat_log_parts = []
                                    if hp_delta != 0:
                                        stat_log_parts.append(f"체력 {'회복' if hp_delta > 0 else '소모'} ({hp_delta:+d})")
                                    if mp_delta != 0:
                                        stat_log_parts.append(f"마나 {'회복' if mp_delta > 0 else '소모'} ({mp_delta:+d})")

                                    if exp_delta > 0:
                                        leveled = add_exp(int(exp_delta))
                                        stat_log_parts.append(f"경험치 +{exp_delta} 획득")
                                        if leveled:
                                            bot_reply += f"\n\n🎊 **[레벨 업!]** Lv.{stats['level']} 달성! 스탯 포인트 5P 획득!"

                                    save_game()
                                    if stat_log_parts:
                                        bot_reply += f"\n\n📊 **[상태 변동]** {', '.join(stat_log_parts)}되었습니다! (현재 HP: {stats['hp']}/{stats['max_hp']}, MP: {stats['mp']}/{stats['max_mp']})"
                                except Exception:
                                    pass
