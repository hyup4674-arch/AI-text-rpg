...
class SyncTextRPGResponse(BaseModel):
    narrative: str = Field(
        description="답변은 반드시 한글로만 합니다 .주사위 눈의 결과와 플레이어의 선택을 반영한 핵심 위주의 현장 묘사 및 서사. 적과 조우하여 전투 행동을 고른 경우, 매 라운드 쪼개지 말고 주사위와 스탯 계산을 통해 전투의 시작부터 끝(승리, 보상, 혹은 피해 후 마무리)까지 한 번에 간결하게 결판을 내어 묘사하세요."
    )
