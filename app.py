...
class SyncTextRPGResponse(BaseModel):
    narrative: str = Field(
        description="답변은 반드시 한글로만 합니다 .주사위 눈의 결과와 플레이어의 선택을 반영한 핵심 위주의 현장 묘사 및 서사. 적과 조우하여 전투 행동을 고른 경우, 매 라운드 쪼개지 말고 주사위와 스탯 계산을 통해 적의 hp가 0이 될될때까지 시작부터 끝(승리, 보상, 혹은 피해 후 마무리)까지 한 번에 간결하게 결판을 내어 묘사하세요."
    )
...
def call_ai_sync_text(user_action, dice_val):
    ...
    system_instruction = (
        ...
        "3. 전투 상황 규칙 (매우 중요): 적을 만나서 공격이나 전투 관련 선택지를 고르면, 매 라운드마다 번거롭게 선택지를 주지 말고, 이번 한 번의 선택과 주사위 결과 및 캐릭터 스탯을 바탕으로 **전투의 처음부터 끝(적 격파, 보상 획득 혹은 도망 등)까지의 결과와 결말을 간결하게 한 번에 마무리**하여 서사로 작성하세요.\n"
    ...
    if ai_provider == "Google Gemini":
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                max_tokens=4096,  # 👈 Groq 토큰 잘림 오류 방지용으로 4096으로 상향 조정
                temperature=0.7,
            )
