...
st.title("⚔️ 판타지 텍스트 RPG 게임 마스터 (전투 후 자동 선택지 연동 버전)")
st.markdown(
    "로컬 전투 종료 시 자동으로 AI가 다음 스토리와 선택지를 생성해 주는 텍스트"
    " RPG 공간입니다."
)
...
# ⚔️ [로컬 파이썬 전투 처리 함수 (전투 종료 시 AI 자동 호출 추가)]
def process_combat_action(action):
  ...
  if enemy["hp"] <= 0:
    ...
    # 📌 전투 종료 후 AI에게 자동으로 다음 스토리와 선택지 요청
